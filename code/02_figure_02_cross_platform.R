...
build_figure_02 <- function(root = find_project_root(), output_dir = NULL) {
  ...
  build_bland_altman_panel <- function(
    ...
    ggplot(bland_altman_df, aes(x = mean_xy, y = diff_yx)) +
      ...
      geom_hline_compat(
        yintercept = c(loa_low, loa_high),
        color = loa_line_color,
        linetype = "dashed",
        width = loa_line_width
      ) +
  ...
  q_stats <- agreement_sheet %>% filter(Parameter == "Qmax") %>% slice(1)
  vv_stats <- agreement_sheet %>% filter(Parameter == "VV") %>% slice(1)
  ...
  panel_a <- build_scatter_panel(
    ...
    x_col = "Oneplus_Qmax",
    y_col = "Redmi_Qmax",
    x_label = "OnePlus Qmax (mL/s)",
    y_label = "Redmi Qmax (mL/s)",
    panel_title = "Same-void cross-device concordance (Qmax)",
    panel_tag = "A",
    label_text = format_stats_label(c(
      paste0("CCC = ", fmt_ccc(q_stats$CCC)),
      paste0("95% CI: ", fmt_ci(q_stats$CCC_low), " to ", fmt_ci(q_stats$CCC_high)),
      paste0("sMAPE = ", fmt_percent(q_stats$MAPE_mean))
    )),
  ...
  panel_b <- build_bland_altman_panel(
    x = clinical$Oneplus_Qmax,
    y = clinical$Redmi_Qmax,
    bias = q_stats$Bias,
    loa_low = q_stats$LoA_low,
    loa_high = q_stats$LoA_high,
    x_label = "Mean of two measurements (mL/s)",
    y_label = "Redmi - OnePlus (mL/s)",
    panel_title = "Same-void cross-device Bland-Altman (Qmax)",
    panel_tag = "B",
    label_text = format_stats_label(c(
  ...
  panel_c <- build_scatter_panel(
    ...
    x_col = "Oneplus_VV",
    y_col = "Redmi_VV",
    x_label = "OnePlus VV (mL)",
    y_label = "Redmi VV (mL)",
    panel_title = "Same-void cross-device concordance (VV)",
    panel_tag = "C",
    label_text = format_stats_label(c(
      paste0("CCC = ", fmt_ccc(vv_stats$CCC)),
      paste0("95% CI: ", fmt_ci(vv_stats$CCC_low), " to ", fmt_ci(vv_stats$CCC_high)),
      paste0("sMAPE = ", fmt_percent(vv_stats$MAPE_mean))
    )),
  ...
  panel_d <- build_bland_altman_panel(
    x = clinical$Oneplus_VV,
    y = clinical$Redmi_VV,
    bias = vv_stats$Bias,
    loa_low = vv_stats$LoA_low,
    loa_high = vv_stats$LoA_high,
    x_label = "Mean of two measurements (mL)",
    y_label = "Redmi - OnePlus (mL)",
    panel_title = "Same-void cross-device Bland-Altman (VV)",
    panel_tag = "D",
    label_text = format_stats_label(c(
  ...
  output_png <- file.path(output_dir, "Figure2_cross_platform.png")
  ...
  panel_outputs <- save_panel_plot_outputs(
    panel_specs = list(
      list(plot_object = panel_a, panel_id = "A", file_stub = "qmax_concordance", row = 1, col = 1),
      list(plot_object = panel_b, panel_id = "B", file_stub = "qmax_bland_altman", row = 1, col = 2),
      list(plot_object = panel_c, panel_id = "C", file_stub = "vv_concordance", row = 2, col = 1),
