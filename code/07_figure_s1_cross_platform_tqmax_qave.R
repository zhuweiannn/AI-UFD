...
build_figure_s1 <- function(root = find_project_root(), output_dir = NULL) {
  ...
  agreement_sheet <- read_excel_safe(results_path, sheet = "A_layer_phone_agreement")

  metric_specs <- list(
    list(parameter = "Qave", x_col = "Oneplus_Qave", y_col = "Redmi_Qave", unit = "mL/s", point_color = "#7876B1FF", scatter_tag = "A", bland_tag = "B"),
    list(parameter = "Tqmax", x_col = "Oneplus_Tqmax", y_col = "Redmi_Tqmax", unit = "s", point_color = "#E18727FF", scatter_tag = "C", bland_tag = "D")
  )
  ...
  metric_panels <- lapply(metric_specs, function(spec) {
    stats_row <- agreement_sheet %>% filter(Parameter == spec$parameter) %>% slice(1)

    scatter_panel <- build_scatter_panel(
      df = clinical,
      x_col = spec$x_col,
      y_col = spec$y_col,
      x_label = sprintf("OnePlus %s (%s)", spec$parameter, spec$unit),
      y_label = sprintf("Redmi %s (%s)", spec$parameter, spec$unit),
      panel_title = sprintf("Same-void cross-device concordance (%s)", spec$parameter),
      panel_tag = spec$scatter_tag,
      label_text = format_stats_label(c(
        paste0("CCC = ", fmt_ccc(stats_row$CCC)),
        paste0("95% CI: ", fmt_ci(stats_row$CCC_low), " to ", fmt_ci(stats_row$CCC_high)),
        paste0("sMAPE = ", fmt_percent(stats_row$MAPE_mean))
      )),
    ...
    bland_panel <- build_bland_altman_panel(
      x = clinical[[spec$x_col]],
      y = clinical[[spec$y_col]],
      bias = stats_row$Bias,
      loa_low = stats_row$LoA_low,
      loa_high = stats_row$LoA_high,
      x_label = sprintf("Mean of two measurements (%s)", spec$unit),
      y_label = sprintf("Redmi - OnePlus (%s)", spec$unit),
      panel_title = sprintf("Same-void cross-device Bland-Altman (%s)", spec$parameter),
  ...
  output_png <- file.path(output_dir, "FigureS1_cross_platform_tqmax_qave.png")
  ...
  panel_dir <- panel_output_dir_for_figure(output_png)
  stale_panel_paths <- Sys.glob(file.path(
    panel_dir,
    paste0(tools::file_path_sans_ext(basename(output_png)), "_panel_*")
  ))
  if (length(stale_panel_paths) > 0) {
    unlink(stale_panel_paths, force = TRUE)
  }

  panel_outputs <- save_panel_plot_outputs(
    panel_specs = list(
      list(plot_object = metric_panels[[1]]$scatter, panel_id = "A", file_stub = "qave_concordance", row = 1, col = 1),
      list(plot_object = metric_panels[[1]]$bland, panel_id = "B", file_stub = "qave_bland_altman", row = 1, col = 2),
      list(plot_object = metric_panels[[2]]$scatter, panel_id = "C", file_stub = "tqmax_concordance", row = 2, col = 1),
      list(plot_object = metric_panels[[2]]$bland, panel_id = "D", file_stub = "tqmax_bland_altman", row = 2, col = 2)
    ),
