...
build_figure_s3 <- function(root = find_project_root(), output_dir = NULL) {
  ...
  metric_specs <- list(
    list(
      parameter = "Qmax",
      summary_metric = "Qmax",
      y_col = "AI_OnePlus_Qmax",
      truth_col = "Truth_Qmax_ml_s",
      unit = "mL/s",
      point_color = "#1F5AA6",
      scatter_tag = "A",
      bland_tag = "B",
      scatter_stub = "oneplus_qmax_concordance",
      bland_stub = "oneplus_qmax_bland_altman",
      y_label = "OnePlus Qmax (mL/s)",
      bland_y_label = "OnePlus - truth (mL/s)"
    ),
    list(
      parameter = "VV",
      summary_metric = "VV",
      ...
      scatter_tag = "C",
      bland_tag = "D",
      scatter_stub = "oneplus_vv_concordance",
      bland_stub = "oneplus_vv_bland_altman",
      y_label = "OnePlus VV (mL)",
      bland_y_label = "OnePlus - truth (mL)"
    )
  )
  ...
  metric_panels <- lapply(metric_specs, function(spec) {
    stats_row <- bench_summary %>% filter(Metric == spec$summary_metric, Device == "OnePlus") %>% slice(1)
    ...
    scatter_panel <- build_scatter_panel(
      ...
      panel_title = sprintf("OnePlus bench concordance (%s)", spec$parameter),
    ...
    bland_panel <- build_bland_altman_panel(
      ...
      panel_title = sprintf("OnePlus bench Bland-Altman (%s)", spec$parameter),
  ...
  output_png <- file.path(output_dir, "FigureS3_bench_oneplus_qmax_vv.png")
